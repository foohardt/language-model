 ...
 * For current purpose function loadModel() was adjusted to fetch pre-trained model.
 *
 */
...
export class SaveableLSTMTextGenerator extends LSTMTextGenerator {
  ...
  async loadModel() {
    const modelURL =
      "http://public.beuth-hochschule.de/~s85393/deep-learning/language-model/model.json";
    this.model = await tf.loadLayersModel(modelURL);
    if (this.model) {
      console.log(`Loaded model from ${modelURL}`);
    } else {
      throw new Error(`Cannot find model at ${modelURL}. `);
    }
  }
